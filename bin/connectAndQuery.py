...
def safe_connect_to_database(queryA, queryB):
    if (queryA or "").lstrip().lower().startswith(("update","insert","delete","replace","alter","create","drop")):
        ...
        raise RuntimeError("safe_connect_to_database is SELECT-only; use insert_to_database for DML")
